...
const api = [
  "getResource",
  ...Object.values(flatRouteMap).flatMap(Object.values),
].reduce((a, b) => {
  a[b] = async (...args) => {
    ...
    try {
      ...
      if (isPlainObject(data) && "results" in data)
        if (
          isPlainObject(data.results[0]) &&
          ["name", "url"].every((key) => key in data.results[0])
        ) {
...
export default Object.assign((a, b) => api[flatRouteMap[a][b]], {
  ...api,
