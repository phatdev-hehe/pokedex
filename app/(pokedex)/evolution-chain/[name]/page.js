import { Checkbox, Link, table, ul } from "@/components";
...
export const evolutionChainTable = (evolution_chain) => {
  /** @type EvolutionChain */
  ...
  const chainTable = (chain) =>
    table(
      ...
      [
          ...
          chain.evolution_details.map((evolutionDetail) => {
            ...
            return ul(
              ...[
                ...
                [
                  "held_item",
                  <Link href={`/item/${heldItem}`}>{titleCase(heldItem)}</Link>,
                ],
                ["item", <Link href={`/item/${item}`}>{titleCase(item)}</Link>],
                [
                  "known_move",
                ...
                [
                  ...
                  <Checkbox checked={evolutionDetail.needs_overworld_rain} />,
                ],
                [
                  "party_species",
        ...
        ["evolves_to", chain.evolves_to.map(chainTable)],
      ]
    );
  ...
  return table(undefined, [
    ...
    ["chain", chainTable(evolutionChain.chain)],
  ]);
};

export default Page(({ context }) => evolutionChainTable(context.data));
